...
for i in range(y2.shape[0]):
    ...
    t2 = x[4, i]
    y2[
        i] = px11 * x1 ** 2 + px22 * x2 ** 2 + px33 * x3 ** 2 + pt1 * t1 + pt2 * t2 + px1 * x1 + px2 * x2 + px3 * x3 + px12 * x1 * x2 + px13 * x1 * x3 + px23 * x2 * x3 + px00
    if (t1 == 1 or t2 == 1):
        t1 = 0
        t2 = 0
        former.append(y[i])
...
np.savetxt("铅钡氧化铁预测.csv", ans, delimiter=',')
print(popt)
