...
param_bounds=([-np.inf,-np.inf,-np.inf,20,20,-np.inf,-np.inf,-np.inf,-np.inf,-np.inf,-np.inf,-np.inf],[np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf])#设定B和n的下界和上界。其中B为负无穷到正无穷，n为0到1


popt, pcov = curve_fit(fun, x, y,method='trf',bounds=param_bounds)
...
np.savetxt("铅钡氧化铅预测.csv",ans,delimiter=',')
print(popt)