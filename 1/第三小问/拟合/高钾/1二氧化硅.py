...
data = pd.read_excel("高钾二氧化硅.xlsx")
...
param_bounds=([-np.inf,-np.inf,-np.inf,18,18,-np.inf,-np.inf,-np.inf,-np.inf,-np.inf,-np.inf,-np.inf],[np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf,np.inf])#设定B和n的下界和上界。其中B为负无穷到正无穷，n为0到1


popt, pcov = curve_fit(fun, x, y,bounds=param_bounds)
...
np.savetxt("高钾二氧化硅预测.csv",ans,delimiter=',')
