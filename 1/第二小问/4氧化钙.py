...
predata = pd.DataFrame(predata, columns=['氧化钙', '风化', '类型'])

model = ols("氧化钙 ~风化+类型+风化:类型", data=predata)
mydata = model.fit()
...
print(ttest_ind(gaojia,qianbei))

print(ttest_ind(gaojianei1,gaojianei2))

print(ttest_ind(qianbeinei1,qianbeinei2))

self_JBtest(gaojia)
self_JBtest(qianbei)

self_JBtest(gaojianei1)
self_JBtest(gaojianei2)

self_JBtest(qianbeinei1)
self_JBtest(qianbeinei2)


print(np.mean(gaojia),np.mean(qianbei),np.mean(gaojia)-np.mean(qianbei))

print(np.mean(gaojianei1),np.mean(gaojianei2),np.mean(gaojianei1)-np.mean(gaojianei2))


print(np.mean(qianbeinei1),np.mean(qianbeinei2),np.mean(qianbeinei1)-np.mean(qianbeinei2))
